...
其中 `agentic-with-cc` 通过 Anthropic 官方原生安装脚本安装 Claude Code；`agentic-with-copilot` 通过 GitHub CLI 官方 apt 仓库安装 `gh`，再用官方 `gh extension install github/gh-copilot` 安装 Copilot CLI。运行时仍需按各工具要求完成认证。
