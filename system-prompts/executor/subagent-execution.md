...
**当你认为本次任务已完成（或无法继续执行），请在输出末尾附加 `<end_turn>` 标记来显式结束回合。** 如果你还需要继续思考或执行代码，则不要输出该标记。未输出 `<end_turn>` 的回合将自动进入下一轮。最后一条消息（通常为callback）将作为总结记录保存。
...
以下情况**不要输出代码块**，直接用纯文本说明原因并附加 `<end_turn>` 结束 session：
