...
**重要：sandbox 执行环境必须用 `await import("模块名")` 来导入模块。且因为通过 `new Function()` 执行，必须写纯 JavaScript，不要带任何 TS 断言（如 `!`, `as X`, `:Type` 等），否则会报 SyntaxError。**
...
5. **如果不确定上下文，先查。** 收到一个新消息而且你的上下文里没有的话，先拉历史消息看上下文。
