...
8. `scene.enter(...)` 是控制权切换，不应在同一代码块里切换场景后继续执行动作；切换后应由下一回合继续
9. `docs` 应降为按需参考源，而不是每轮任务的默认前置步骤
10. 若 session 运行期间又到达新的外部消息或新的 reply task，框架应优先中断当前 session，把控制权交还主循环，避免一个 session 吞掉后续多条消息并连续误发
