...
动态读取这几个具体方法的**完整类型定义（包含详尽的 JSDoc、多行说明、具体参数和示例）**，像“翻开说明书的这一页”一样，注入给 LLM

**然后把这一轮他生成的代码直接忽视**，带上文档之后，重新生成一次代码（相当于从历史N-1处，fork了一份消息记录）

为什么要删掉第一次的呢。就是因为作为框架，你有什么最优雅的方式来推测她“需要什么文档”呢？那当然是她盲写的代码。

你不需要另一个LLM来进行推测，预测，任何东西，你只需要看他生成了什么。这就是他自发需要使用的东西
