...
export class ReplyPipeline {
    ...
    private buildDirectPrompt(messages: Message[], route: ModelRouteResult, sceneFocus: string, latentMemory: string): string {
        ...
        return [
            ...
            `建议模型: ${route.model}`,
            `群组: chatId=${messages[0]?.chatId ?? "unknown"}`,
            "所有 API（ctx.tg / memory / actions / skills）已直接可用，无需切换场景。",
            "",
            sceneFocus,
            "",
            latentMemory,
    ...
    private buildTopicPrompt(
        ...
        return [
            ...
            `建议模型: ${route.model}`,
            `话题: ${topic.label}`,
            `群组: chatId=${topic.chatId}`,
            "所有 API（ctx.tg / memory / actions / skills）已直接可用。",
