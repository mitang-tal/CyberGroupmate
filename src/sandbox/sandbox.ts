...
export class Sandbox extends EventEmitter {
    ...
    private async startPty(): Promise<void> {
        ...
        // 生成 .bashrc（放在隐藏的 sandbox-state 目录下，避免污染 workspace）
        ...
        const bashrcPath = join(sandboxStateDir, ".bashrc");
        const bashrcContent = [
    ...
    private saveEventListeners(): void {
        try {
            const safeChatId = this.chatId.replace(/[^a-zA-Z0-9_\-\.]/g, "_");
            const filePath = join(this.projectRoot, "workspace", "sessions", ".sandbox-state", safeChatId, "event-listeners.json");
    ...
    private saveWebhooks(): void {
        try {
            const safeChatId = this.chatId.replace(/[^a-zA-Z0-9_\-\.]/g, "_");
            const filePath = join(this.projectRoot, "workspace", "sessions", ".sandbox-state", safeChatId, "webhooks.json");
            const dir = dirname(filePath);
            if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
