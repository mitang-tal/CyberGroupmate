...
export function createTelegramClientProxy(
    ...
    /** 两个字符串的 bigram Jaccard 相似度（0~1）。 */
    function bigramJaccard(a: string, b: string): number {
        if (!a && !b) return 1;
        if (!a || !b) return 0;
        const bis = (s: string): Set<string> => {
            const t = normalizeForDedup(s);
            const set = new Set<string>();
            if (t.length <= 1) { if (t) set.add(t); return set; }
            for (let i = 0; i < t.length - 1; i++) set.add(t.slice(i, i + 2));
            return set;
        };
        const A = bis(a), B = bis(b);
    ...
    /** 短消息近义判定门槛：太短的文本 bigram 相似度天然虚高，不走近义判定，
     *  回退到精确匹配，避免误杀 Telegram 拟人 bot 的多段短消息连发。 */
    const NEAR_DUP_MIN_LEN = 10;

    /** 是否近义重复（同一含义、措辞略有差异）。仅在开启去重时生效。 */
    function isNearDuplicate(chatId: string, text: string): boolean {
        if (!deduplicateSentMessages) return false;
        const existing = sentHistory.get(String(chatId));
        if (!existing || existing.size === 0) return false;
        const norm = normalizeForDedup(text);
        if (norm.length < NEAR_DUP_MIN_LEN) return false; // 短消息走精确匹配
        for (const prev of existing) {
            const prevNorm = normalizeForDedup(prev);
            if (!prevNorm) continue;
            // 包含判定：较短一方也需达到门槛，避免「好的」永久拦截「...好的...」
            const shorter = Math.min(norm.length, prevNorm.length);
            if (shorter >= NEAR_DUP_MIN_LEN && (norm.includes(prevNorm) || prevNorm.includes(norm))) return true;
            // bigram 相似度判定：双方都需达到门槛
            if (prevNorm.length >= NEAR_DUP_MIN_LEN && bigramJaccard(norm, prevNorm) >= 0.8) return true;
        }
        return false;
    }

    function shouldBlockDuplicate(chatId: string, text: string): boolean {
        return deduplicateSentMessages && (isDuplicate(chatId, text) || isNearDuplicate(chatId, text));
    }
    ...
    const methods = {
        ...
        sendText: async (chatId: number | string, text: unknown, opts?: Record<string, unknown>) => {
            ...
            // ── 拟人化：发送前按字符数延迟（模拟打字节奏），50ms/字符，clamp [500,5000]ms
            // 规则来自米汤的人设/todo（模型不听 prompt，这里代码层硬保证）
            const typingDelayMs = Math.min(5000, Math.max(500, plainText.length * 50));
            try {
        ...
        sendTyping: async (chatId: number | string) => {
            // 拟人化 typing 已由 sendText 自动处理（发送前 sendTyping + 打字延迟）。
            // 这里改为 no-op：避免模型在处理/卡顿期间显式反复调用 sendTyping，
            // 导致"正在输入"在非打字时也持续显示（直到 turn 结束才消失）。
            env.emitOutput(`[Telegram] sendTyping (no-op, typing auto-handled by sendText) chat=${String(chatId)}`);
        },
