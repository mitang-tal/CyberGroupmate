...
<div class="card bg-base-100 mb-4">
  <div class="card-body p-4">
    <div class="flex justify-between items-center mb-2">
      ...
      <button class="btn btn-xs btn-primary" onclick={showEnqueueModal}>手动入队</button>
    </div>
          ...
          {:else}
            {#each activeList as e}
              <tr class="queue-row" class:is-blocked={e.blocked}>
                ...
                <td>{#if e.blocked}<span class="badge badge-xs badge-error">阻塞</span>{:else}<span class="badge badge-xs badge-success">活跃</span>{/if}</td>
                <td>
                  <div class="flex gap-1">
                    <button class="btn btn-xs btn-ghost" onclick={() => boost(e.chatId)}>⬆</button>
                    <button class="btn btn-xs btn-ghost text-error" onclick={() => remove(e.chatId)}>✕</button>
                  </div>
                </td>
              </tr>
            {/each}
...
<div class="collapse collapse-arrow bg-base-100">
  ...
  <div class="collapse-title text-sm font-medium">
    📜 已处理 (Dequeued) <span class="badge badge-sm badge-ghost ml-2">{dequeuedList.length}</span>
  </div>
  <div class="collapse-content">
    <div class="overflow-x-auto">
      <table class="table table-xs">
        <thead><tr>
          <th>ChatId</th><th>优先级</th><th>来源</th><th>Stickiness</th><th>出队时间</th>
        </tr></thead>
        <tbody>
          ...
          {:else}
            {#each dequeuedList as d}
              <tr class="dequeued-row">
                ...
                <td class="opacity-60">{new Date(d.dequeuedAt).toLocaleTimeString()}</td>
              </tr>
            {/each}
          {/if}
        </tbody>
...
<style>
.queue-row :global(td) { vertical-align: middle; }
.queue-row.is-blocked { opacity: 0.5; }

.priority-high { color: var(--color-error); font-weight: 700; }
.priority-mid { color: var(--color-warning); font-weight: 600; }
.priority-low { color: var(--color-success); }

.stickiness-CORE { color: var(--color-error); }
.stickiness-FAMILIAR { color: var(--color-warning); }
.stickiness-ACQUAINTANCE { color: var(--color-success); }
.stickiness-STRANGER { color: color-mix(in srgb, var(--color-base-content) 60%, transparent); }

.dequeued-row :global(td) { opacity: 0.7; }
</style>
