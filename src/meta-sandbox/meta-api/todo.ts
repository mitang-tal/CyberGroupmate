...
export interface TodoSetInput {
    ...
    content: string;
    bindingId: string;
    type: string; // 必填：task|policy|preference|experience|observation|log
...
export function createTodoApi(memory: TodoMemory) {
    return {
        ...
        set: async (input: TodoSetInput) => {
            ...
            return {
                bindingId,
                ...memory.todoUpsert(bindingId, key, type, content, resolveTodoDueAt(input)),
            };
        },
        update: async (key: string, input: TodoUpdateInput, bindingId: string) => {
            ...
            if (!nextType) throw new Error("todo update requires a type either in input or existing item");
